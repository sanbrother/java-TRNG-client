...
import org.random.rjgodoy.util.Log;
import org.random.rjgodoy.util.LogFactory;
...
public class MH_HttpClient {
	...
	public final static String JVM  = "Java/"
		                              + System.getProperty("java.runtime.version").replaceAll("[()<>@,;:\\\\\"/\\[\\]?={}\\p{Blank}\\p{Cntrl}]","_")
		                              + " JVM/"
		                              +System.getProperty("java.vm.version").replaceAll("[()<>@,;:\\\\\"/\\[\\]?={}\\p{Blank}\\p{Cntrl}]","_")
																	+ "("+(System.getProperty("java.vm.vendor").replaceAll("[()\\p{Cntrl}]","").replaceAll("[.,]"," ")).trim()+")";
	public final static String VERSION  = "0.1.0";
	private final static String AUTHOR  = "rjgodoy@fich.unl.edu.ar";
	...
	MH_HttpClient() {
		...
		String user          = System.getProperty(MH_SecureRandom.USER);
		String so_timeout    = System.getProperty(MH_SecureRandom.TIMEOUT);
		String cert_file     = System.getProperty(MH_SecureRandom.CERTFILE);
		String ssl_provider  = System.getProperty(MH_SecureRandom.SSL_PROVIDER);
		String ssl_protocol  = System.getProperty(MH_SecureRandom.SSL_PROTOCOL);
		String max_redirects = System.getProperty(MH_SecureRandom.MAX_REDIRECTS);
		...
		if (cert_file!=null) {
			CERT_FILE=cert_file;
			URI cert_file_uri = null;
			try {
				cert_file_uri = this.getClass().getClassLoader().getResource(cert_file).toURI();
			} catch (URISyntaxException e) {
				LOG.error(e);
				CERT_FILE=null;
			}
			if (cert_file_uri!=null&&!cert_file_uri.isOpaque()&&!new File(cert_file_uri).exists()){
   ...
   */
	...
	long checkQuota() {
		...
		try {
			...
			return quota=Long.parseLong(response.trim());
		} catch (Exception e) {
			logException(e);
	 ...
	 */
	int nextBytes(byte[] bytes,int offset,int length) {
	...
	private String get(String request_uri,int nredirects) throws IOException {
		...
		String request =
			"User-Agent: "+AUTHOR+"/"+VERSION+" ("+USER+") "+JVM+"\r\n"+
			"Accept: text/plain\r\n"+
  ...
  //Returns the text/plain body of an HTTP request
	...
	private Socket createSocket()
		...
		if (ssl_factory==null) {
			...
			//if this MH_TRNG is the default SecureRandom algorithm
			//we need another SecureRandom algorithm for initializing the SSLContext
			SecureRandom srandom = null;
			Provider[] providers = Security.getProviders();
			if (providers[0].getName().equalsIgnoreCase("RJGODOY")) {
				outer: for (int i=1;i<providers.length;i++)
					for (Service service:providers[i].getServices())
						if (service.getType().equals("SecureRandom")) {
							LOG.warn("Cannot instantiate: we are the default provider (java bug)");
							//this doesn't works because of a bug with Java
							srandom=SecureRandom.getInstance(service.getAlgorithm(),service.getProvider().getName());
							break outer;
					  }
			}
			...
			if (CERT_FILE!=null) {
				...
				InputStream inStream = this.getClass().getClassLoader().getResourceAsStream(CERT_FILE);
				...
				LOG.info("(Server Certificate) "+ cert.getSubjectDN().getName());
				...
				LOG.warn("No server certificate file given");
				ctx.init(null,null,srandom);
			}
