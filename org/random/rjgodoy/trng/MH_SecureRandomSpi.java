...
/** <tt>SecureRandomSpi</tt> implementation that uses the True Random Number generator from
...
public class MH_SecureRandomSpi extends SecureRandomSpi {
	...
	private final static Set<String>    unique_log_str = new HashSet<String>();

	private final boolean uses_deamon;
	...
	public MH_SecureRandomSpi() throws NoSuchAlgorithmException, NoSuchProviderException{
		...
		synchronized (MH_SecureRandom.class) {
			...
			if (LOG.isInfoEnabled()) {
				String str = "mode="+this.mode+"; fallback="+this.fallback;
				if (unique_log_str.add(str)) LOG.info(str);
			}
		}
		...
		if (this.mode==GeneratorMode.TRNG&&this.fallback==FallbackPolicy.TRNG) {
			...
			if (prng_provider!=null)
				...
				outer: for (Provider provider:Security.getProviders())
					if (!provider.getName().equals("RJGODOY")){
						for (Service service:provider.getServices())
							if (service.getType().equals("SecureRandom")) {
								prng = SecureRandom.getInstance(service.getAlgorithm(), provider.getName());
								break outer;
							}
				  }
			  if (prng==null) prng = new SecureRandom(); //this causes an endless loop
			}
			if (prng!=null) {
				if (LOG.isInfoEnabled()) synchronized (MH_SecureRandom.class) {
					String str = "PRNG ["+prng.getProvider()+":"+prng.getAlgorithm()+"]";
					if (unique_log_str.add(str)) LOG.info(str);
				}
			}
		}
							...
							while (!isInterrupted()) {
								...
								try {
									...
									if (quota==FULL_QUOTA) {
										...
										while (blockSize>=256) {//8192
												int length = fetch(256);
												if (length<=0) break;
												blockSize-=length;
											}
										if (blockSize>0)
